class DeductionNode {
  static set Clipboard(s) {
    try {
      window.localStorage.setItem("proofJSClipboard", s);
    } catch {
      DeductionNode.ClipboardContent = s;
    }
  }

  static get Clipboard() {
    try {
      return window.localStorage.getItem("proofJSClipboard");
    } catch {
      return DeductionNode.ClipboardContent || "";
    }
  }
  ...
  renderOn(target) {
    ...
    elt.input = DeductionNode.input(this.label, (i) => {
      if (!this.parentNode) return Math.max(i.value.length, 1);
        ...
        var myShare =
          this.parentNode.label.length / this.parentNode.forest.length;
    ...
    elt.addRule = () => {
      ...
      if (childElt) {
        ...
        elt.rule = DeductionNode.input(this.ruleContent, (i) =>
    ...
    elt.input.addEventListener("keydown", (e) => {
      if (e.code == "KeyZ" && e.ctrlKey) e.preventDefault();
    });
    elt.input.addEventListener("keydown", (e) => {
      if (e.code == "KeyC" && e.shiftKey && e.ctrlKey) e.preventDefault();
    });
    elt.input.addEventListener("keydown", (e) => {
      if (e.code == "KeyV" && e.shiftKey && e.ctrlKey) e.preventDefault();
    });
    elt.input.addEventListener("keydown", (e) => {
      if (e.code == "KeyX" && e.shiftKey && e.ctrlKey) e.preventDefault();
    });
    elt.input.addEventListener("keyup", (e) => {
      ...
      if (e.code == "Enter" && e.ctrlKey && e.shiftKey) {
        ...
        e.preventDefault();
        this.replace(
          new DeductionNode(JSON.parse(DeductionNode.Clipboard)).scrubIdent()
        );
      }
  ...
  decorate(obj) {
    ...
    var i = 0;
    if (typeof obj.forest != "undefined")
